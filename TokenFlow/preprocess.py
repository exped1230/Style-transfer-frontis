...
class Preprocess(nn.Module):
    def __init__(self, device, opt, hf_key=None):
        ...
        self.unet = UNet2DConditionModel.from_pretrained(model_key, subfolder="unet", revision="fp16",
                                                   torch_dtype=torch.float16).to(self.device)
        self.paths, self.frames, self.latents = self.get_data(opt.data_path, opt.n_frames, opt.H, opt.W)
    ...
    def get_data(self, frames_path, n_frames, H, W):
        # load frames
        ...
        if frames[0].size[0] == frames[0].size[1]:
            frames = [frame.resize((W, H), resample=Image.Resampling.LANCZOS) for frame in frames]
...
if __name__ == "__main__":
    ...
    opt.data_path = os.path.join('./data', file_name + '_inpaint.mp4')
