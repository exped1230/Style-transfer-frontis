...
class TokenFlow(nn.Module):
    def __init__(self, config):
        ...
        self.scheduler = DDIMScheduler.from_pretrained('./SD-v1-5', subfolder="scheduler")
        self.scheduler.set_timesteps(config["n_timesteps"], device=self.device)

        pipe = DiffusionPipeline.from_pretrained('./SD-v1-5', scheduler=self.scheduler, torch_dtype=torch.float16).to("cuda")
        
        # 加进来以后对batch size和n_frames有要求，目前测试出来只有16+64的配置可以
        if config['use_hidiffusion']:
            apply_hidiffusion(pipe)
            pipe.info['pipeline']._num_timesteps = config["n_timesteps"]
    ...
    def edit_video(self):
        ...
        save_video(edited_frames, '../Track-Anything/transferred_video.mp4', fps=20)
        print('Done!')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config_path', type=str, default='configs/config_custom.yaml')
    ...
    file_name = open('../Track-Anything/info.txt', 'r').readline().strip().split('.')[0]
    config["data_path"] = os.path.join('./data', file_name + '_inpaint')
